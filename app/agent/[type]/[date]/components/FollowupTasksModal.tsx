  ...
  const handlePrint = useCallback(async () => {
    try {
      ...
      tasks.forEach((taskGroup, groupIdx) => {
        ...
        doc.setFontSize(10);
        doc.text(`Call Time - ${taskGroup.call_time}`, 20, y);
        y += 7;

        doc.setFontSize(10);
        doc.text(`Call Duration - ${taskGroup.call_duration}`, 20, y);
        y += 7;
        ...
        if (taskGroup.hawksoft_url && 
          ...
          ensureSpace(taskGroup.hawksoft_url.length + 2);
          y += 3;
          doc.setFont("helvetica", "bold");
          doc.setFontSize(12);
        ...
        taskGroup.tasks.forEach((task) => {
          const taskLines = doc.splitTextToSize(task, usableWidth - 5);
