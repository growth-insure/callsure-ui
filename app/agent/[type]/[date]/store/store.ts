...
export const useAgentCallDetailsStore = create<CallDetailsState>((set) => ({
  calls: [],
  metrics: {
    totalCalls: 0,
    outboundCalls: 0,
  ...
  fetchCallDetails: async (type: string, date: string, extension: string) => {
    set({ isLoading: true, error: null });
    try {
      let url = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/audio_files?select=*&call_date=eq.${date}&extension=eq.${extension}&order=call_time.desc`;
      ...
      // Note: For "followups" type, we fetch all calls and filter client-side
      // because Supabase JSON array filtering for array length is complex
      ...
      // Filter for follow-ups if type is "followups"
      // Only include calls that have follow_up_actions array with at least one item
      if (type === "followups") {
        data = data.filter((call: CallRecord) => 
          call.call_summary?.follow_up_actions && 
          Array.isArray(call.call_summary.follow_up_actions) && 
          call.call_summary.follow_up_actions.length > 0
        );
      }
